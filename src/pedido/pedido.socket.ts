...
@WebSocketGateway({
  cors: {
    origin: "http://localhost:3000", //"https://restaurante-adminsite.ypg4r9.easypanel.host", 
    methods: ["GET", "POST"],
  },
})
  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  avisarNovoPedidoGerado(pedido: Pedido) {
    console.log(
      `WebSocket - Novo pedido gerado: ${JSON.stringify(pedido.restaurante)}: ${JSON.stringify(pedido.pedidoId)}`,
    );
